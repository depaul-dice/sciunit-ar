...
#define AUTHORS                                                          \
	proper_name("Paul Eggert"), proper_name("Mike Haertel"),         \
	    proper_name("David Hayes"), proper_name("Richard Stallman"), \
...
static char const group_format_option[][sizeof "--unchanged-group-format"] = {
	"--unchanged-group-format", "--old-group-format", "--new-group-format",
	"--changed-group-format"
};

static char const line_format_option[][sizeof "--unchanged-line-format"] = {
	"--unchanged-line-format", "--old-line-format", "--new-line-format"
};
...
static struct option const longopts[] = {
	...
	{ "-presume-output-tty", no_argument, NULL,
	  PRESUME_OUTPUT_TTY_OPTION },
	{ 0, 0, 0, 0 }
};
...
int bootstrapMain(int argc, char** argv)
{
	...
	while ((c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1)
	{
		...
		case 'B': ignore_blank_lines = true; break;
		...
		case 'U':
		{
			if (optarg)
			{
				...
				if (*numend)
					try_help("invalid context length '%s'",
					         optarg);
		...
		case 'D':
			specify_style(OUTPUT_IFDEF);
			{
				...
				char* b = xmalloc(
				    ...
				    - 8 /* 5*"%%" + 3*"%c" */);
				sprintf(b, C_ifdef_group_formats, 0, optarg,
				        optarg, 0, optarg, optarg, 0, optarg,
				        optarg, optarg);
				for (i = 0; i < sizeof group_format /
				                    sizeof group_format[0];
				     i++)
				{
					specify_value(&group_format[i], b,
					              "-D");
					b += strlen(b) + 1;
				}
			}
		...
		case 'e': specify_style(OUTPUT_ED); break;
		...
		case 'f': specify_style(OUTPUT_FORWARD_ED); break;

		case 'F': add_regexp(&function_regexp_list, optarg); break;
		...
		case 'H': speed_large_files = true; break;

		case 'i': ignore_case = true; break;

		case 'I': add_regexp(&ignore_regexp_list, optarg); break;
		...
		case 'n': specify_style(OUTPUT_RCS); break;

		case 'N': new_file = true; break;
		...
		case 'P': unidirectional_new_file = true; break;

		case 'q': brief = true; break;

		case 'r': recursive = true; break;

		case 's': report_identical_files = true; break;

		case 'S': specify_value(&starting_file, optarg, "-S"); break;

		case 't': expand_tabs = true; break;

		case 'T': initial_tab = true; break;
		...
		case 'w': ignore_white_space = IGNORE_ALL_SPACE; break;
		...
		case 'y': specify_style(OUTPUT_SDIFF); break;
		...
		case HORIZON_LINES_OPTION:
			numval = strtoumax(optarg, &numend, 10);
			if (*numend)
				try_help("invalid horizon length '%s'",
				         optarg);
			horizon_lines =
			    MAX(horizon_lines, MIN(numval, LIN_MAX));
			break;
		...
		case LEFT_COLUMN_OPTION: left_column = true; break;

		case LINE_FORMAT_OPTION:
			specify_style(OUTPUT_IFDEF);
			for (i = 0;
			     i < sizeof line_format / sizeof line_format[0];
			     i++)
				specify_value(&line_format[i], optarg,
				              "--line-format");
			break;
		...
		case NORMAL_OPTION: specify_style(OUTPUT_NORMAL); break;
		...
		case STRIP_TRAILING_CR_OPTION: strip_trailing_cr = true; break;
		...
		case TABSIZE_OPTION:
			numval = strtoumax(optarg, &numend, 10);
			if (!(0 < numval &&
			      numval <= SIZE_MAX - GUTTER_WIDTH_MINIMUM) ||
			    *numend)
		...
		case NEW_LINE_FORMAT_OPTION:
			...
			specify_value(&line_format[c], optarg,
			              line_format_option[c]);
			break;
		...
		case CHANGED_GROUP_FORMAT_OPTION:
			...
			specify_value(&group_format[c], optarg,
			              group_format_option[c]);
			break;

		case COLOR_OPTION: specify_colors_style(optarg); break;

		case COLOR_PALETTE_OPTION: set_color_palette(optarg); break;

		case PRESUME_OUTPUT_TTY_OPTION:
			presume_output_tty = true;
			break;

		default: try_help(NULL, NULL);
		}
		prev = c;
	}
	...
	{
		/* Maximize first the half line width, and then the gutter
		   width, according to the following constraints:
	...
	{
		for (i = 0; i < sizeof line_format / sizeof line_format[0];
		     i++)
			if (!line_format[i])
				line_format[i] = "%l\n";
		if (!group_format[OLD])
			group_format[OLD] = group_format[CHANGED]
			                        ? group_format[CHANGED]
			                        : "%<";
		if (!group_format[NEW])
			group_format[NEW] = group_format[CHANGED]
			                        ? group_format[CHANGED]
			                        : "%>";
	...
	if (from_file)
	{
		if (to_file)
			...
			{
				int status = compare_files(NULL, from_file,
				                           argv[optind]);
	...
	else
	{
		if (to_file)
			...
			{
				int status =
				    compare_files(NULL, argv[optind], to_file);
				if (exit_status < status)
					exit_status = status;
			}
		else
		{
			...
			{
				if (argc - optind < 2)
					try_help("missing operand after '%s'",
					         argv[argc - 1]);
				else
					try_help("extra operand '%s'",
					         argv[optind + 2]);
			}

			exit_status = compare_files(NULL, argv[optind],
			                            argv[optind + 1]);
		}
	}
...
static void add_regexp(struct regexp_list* reglist, char const* pattern)
{
	...
	else
	{
		...
		bool multiple_regexps = reglist->multiple_regexps =
		    regexps != 0;
		size_t newlen = reglist->len =
		    len + 2 * multiple_regexps + patlen;
...
static void summarize_regexp_list(struct regexp_list* reglist)
{
	if (reglist->regexps)
	{
		/* At least one regexp was specified.  Allocate a fastmap for
		 * it.  */
		reglist->buf->fastmap = xmalloc(1 << CHAR_BIT);
		if (reglist->multiple_regexps)
		{
			/* Compile the disjunction of the regexps.
			   (If just one regexp was specified, it is already
			   compiled.)  */
			char const* m = re_compile_pattern(
			    reglist->regexps, reglist->len, reglist->buf);
			if (m)
				die(EXIT_TROUBLE, 0, "%s: %s",
				    reglist->regexps, m);
		}
	}
}
...
static char const* const option_help_msgid[] =
    { N_("    --normal                  output a normal diff (the default)"),
      ...
      N_("-c, -C NUM, --context[=NUM]   output NUM (default 3) lines of "
	 "copied context"),
      N_("-u, -U NUM, --unified[=NUM]   output NUM (default 3) lines of "
	 "unified context"),
      N_("-e, --ed                      output an ed script"),
      N_("-n, --rcs                     output an RCS format diff"),
      N_("-y, --side-by-side            output in two columns"),
      N_("-W, --width=NUM               output at most NUM (default 130) "
	 "print columns"),
      N_("    --left-column             output only the left column of common "
	 "lines"),
      N_("    --suppress-common-lines   do not output common lines"),
      "",
      N_("-p, --show-c-function         show which C function each change is "
	 "in"),
      N_("-F, --show-function-line=RE   show the most recent line matching "
	 "RE"),
      N_("    --label LABEL             use LABEL instead of file name and "
	 "timestamp\n"
	 "                                (can be repeated)"),
      "",
      N_("-t, --expand-tabs             expand tabs to spaces in output"),
      N_("-T, --initial-tab             make tabs line up by prepending a "
	 "tab"),
      N_("    --tabsize=NUM             tab stops every NUM (default 8) print "
	 "columns"),
      N_("    --suppress-blank-empty    suppress space or tab before empty "
	 "output lines"),
      N_("-l, --paginate                pass output through 'pr' to paginate "
	 "it"),
      "",
      N_("-r, --recursive                 recursively compare any "
	 "subdirectories found"),
      ...
      N_("    --ignore-file-name-case     ignore case when comparing file "
	 "names"),
      N_("    --no-ignore-file-name-case  consider case when comparing file "
	 "names"),
      N_("-x, --exclude=PAT               exclude files that match PAT"),
      N_("-X, --exclude-from=FILE         exclude files that match any "
	 "pattern in FILE"),
	 ...
	 "contents"),
      N_("-E, --ignore-tab-expansion      ignore changes due to tab "
	 "expansion"),
	 ...
	 "white space"),
      ...
      N_("-B, --ignore-blank-lines        ignore changes where lines are all "
	 "blank"),
...
#endif
      "",
      N_("-D, --ifdef=NAME                output merged file with '#ifdef "
	 "NAME' diffs"),
      N_("    --GTYPE-group-format=GFMT   format GTYPE input groups with "
	 "GFMT"),
      N_("    --line-format=LFMT          format all input lines with LFMT"),
      N_("    --LTYPE-line-format=LFMT    format LTYPE input lines with LFMT"),
      N_("  These format options provide fine-grained control over the "
	 "output\n"
	 "    of diff, generalizing -D/--ifdef."),
      N_("  LTYPE is 'old', 'new', or 'unchanged'.  GTYPE is LTYPE or "
	 "'changed'."),
    ...
    C    the character C (other characters represent themselves)"),
	 ...
	 "suffix"),
      N_("    --speed-large-files  assume large files and many scattered "
	 "small changes"),
	 ...
	 "FILE(s)."),
      ...
      N_("Exit status is 0 if inputs are the same, 1 if different, 2 if "
	 "trouble."),
      0 };
...
/* Set VAR to VALUE, reporting an OPTION error if this is a
   conflict.  */
static void specify_value(char const** var, char const* value,
                          char const* option)
{
	if (*var && !STREQ(*var, value))
	{
		error(0, 0, _("conflicting %s option value '%s'"), option,
		      value);
		try_help(NULL, NULL);
	}
	*var = value;
...
/* Set the color mode.  */
static void specify_colors_style(char const* value)
{
...
#define DIR_P(f) (S_ISDIR(cmp.file[f].stat.st_mode) != 0)
	...
	if (!((name0 && name1) || (unidirectional_new_file && name1) ||
	      new_file))
	{
...
#define ERRNO_DECODE(desc) (-3 - (desc)) /* inverse of ERRNO_ENCODE */
	...
	for (f = 0; f < 2; f++)
	{
		...
		{
			if (f && file_name_cmp(cmp.file[f].name,
			                       cmp.file[0].name) == 0)
			{
			...
			else if (STREQ(cmp.file[f].name, "-"))
			{
				...
				if (binary && !isatty(STDIN_FILENO))
					set_binary_mode(STDIN_FILENO,
					                O_BINARY);
				if (fstat(STDIN_FILENO, &cmp.file[f].stat) !=
				    0)
				...
				else
				{
					...
					/* POSIX 1003.1-2001 requires current
					   time for stdin.  */
					set_mtime_to_now(&cmp.file[f].stat);
				}
			}
			else if ((no_dereference_symlinks
			              ? lstat(cmp.file[f].name,
			                      &cmp.file[f].stat)
			              : stat(cmp.file[f].name,
			                     &cmp.file[f].stat)) != 0)
				cmp.file[f].desc = ERRNO_ENCODE(errno);
		}
	}
	...
	else if (DIR_P(0) & DIR_P(1))
	{
		...
		if (parent && !recursive)
		{
			...
			status = diff_dirs(&cmp, compare_files);
	         ...
	         (parent && !((S_ISREG(cmp.file[0].stat.st_mode) ||
	                       ...
	                       S_ISLNK(cmp.file[1].stat.st_mode)))))
	{
		if (cmp.file[0].desc == NONEXISTENT ||
		    cmp.file[1].desc == NONEXISTENT)
		{
			/* We have a subdirectory that exists only in one
			 * directory.  */

			if ((DIR_P(0) | DIR_P(1)) && recursive &&
			...
			else
			{
				...
				dir =
				    parent
				        ->file[cmp.file[0].desc == NONEXISTENT]
				        .name;
	         ...
	         S_ISLNK(cmp.file[1].stat.st_mode))
	{
		    ...
		    S_ISLNK(cmp.file[1].stat.st_mode))
		{
			...
			if (status == EXIT_SUCCESS)
			{
				...
				{
					message("Symbolic links %s and %s "
					        "differ\n",
					        cmp.file[0].name,
					        cmp.file[1].name);
					/* This is a difference.  */
		...
		else
		{
			/* We have two files that are not to be compared,
			   because
			   one of them is a symbolic link and the other one is
			   not.  */
	...
	else
	{
		...
		if (cmp.file[0].desc == UNOPENED)
			if ((cmp.file[0].desc =
			         open(cmp.file[0].name, oflags, 0)) < 0)
			{
		...
		if (cmp.file[1].desc == UNOPENED)
		{
			...
			else if ((cmp.file[1].desc =
			              open(cmp.file[1].name, oflags, 0)) < 0)
			{
		...
		if (0 <= cmp.file[1].desc &&
		    cmp.file[0].desc != cmp.file[1].desc &&
